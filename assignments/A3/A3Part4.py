...
def suppressFreqDFTmodel(x, fs, N):
    ...
    freqs = np.arange(N/2) * float(fs) / (N)
    idx = np.argmax(freqs >= 70)
    # print(idx)
    mX[:(idx+1)] = -120
    # print(len(freqs), len(mX), sum(freqs <= 70.))
    # print (len(mX), len(freqs))
    # mX[np.concatenate(([True],(freqs <= 71.)))] = -120
    # pX[np.concatenate(([True],(freqs <= 70.)))] = 0
    y_filt = dftSynth(mX, pX, w.size)*sum(w)
    return(y, y_filt)
    ...
    #
    # f = 70
    # div = float(f)/float(fs)
    # k = int(float(N) * div)  #k bin at 70Hz = N * (f/fs)
    #
    # mxFilt = np.zeros(mx.size)
    # mxFilt.fill(-120)
    # mxFilt[k:] = mx[k:]     #from 0 to k Mgnitude is -120 dB
...
if __name__ == '__main__':
    ...
    nn = np.arange(n)
    fs1 = 10000.
    freqs = (40,100,200,1000)
    tt = nn / fs1
    x_list = [np.cos(tt*2*np.pi * f) for f in freqs]
    xx1 = np.sum(x_list, axis=0)
    y, y_filt = suppressFreqDFTmodel(xx1, fs1, N)
