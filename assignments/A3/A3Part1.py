...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    """
    c1 = fs / float(f1)  # cycles per second
    c2 = fs / float(f2)
    M = int(c1*c2 / gcd(c1,c2))
    X = fft(x[:M])
...
if __name__ == '__main__':
    ...
    f2 = 200
    tt = nn / fs1
    x1 = np.cos(tt*2*np.pi * f1)
    x2 = np.cos(tt*2*np.pi * f2)
    xx1 = x1 + x2
    ans1 = minimizeEnergySpreadDFT(xx1, fs1, f1, f2)
    fs2 = 48000.
    f3 = 300
    f4 = 800
    tt = nn / fs2
    x3 = np.cos(tt*2*np.pi * f3)
    x4 = np.cos(tt*2*np.pi * f4)
    xx2 = x3 + x4
    ans2 = minimizeEnergySpreadDFT(xx2, fs2, f3, f4)
