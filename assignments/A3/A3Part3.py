...
def testRealEven(x):
    ...
    """
    nr_samples = len(x)
    fft_buffer = np.zeros(nr_samples)
    s = int(math.floor(nr_samples/2.))
    if nr_samples % 2 == 0:
        fft_buffer[:s] = x[s:]
    else:
        fft_buffer[:(s+1)] = x[s:]
    fft_buffer[-s:] = x[:s]
    X = fft(fft_buffer)
    if np.all(np.imag(X) < 1e-12):
        isRealEven = True
    else:
        isRealEven = False
    return(isRealEven, fft_buffer, X)


if __name__ == '__main__':
    x1 = np.array([ 2, 3, 4, 3, 2 ])
    ans1 = testRealEven(x1)
    x2 = np.array([1, 2, 3, 4, 1, 2, 3])
    ans2 = testRealEven(x2)
