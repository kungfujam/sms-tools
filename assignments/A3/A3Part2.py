...
def optimalZeropad(x, fs, f):
    ...
    """
    nr_samples = len(x)
    period_len = fs / float(f)
    nr_periods = nr_samples / period_len
    fft_len = int(ceil(nr_periods) * period_len)
    fft_buffer = np.zeros(fft_len)
    s = int(floor(nr_samples/2.))
    if nr_samples % 2 == 0:
        fft_buffer[:s] = x[s:]
    else:
        fft_buffer[:(s+1)] = x[s:]
    fft_buffer[-s:] = x[:s]
    X = fft(fft_buffer)
    mX = 20*np.log10(abs(X[:fft_len/2+1]))
    return mX
...
if __name__ == '__main__':
    ...
    f1 = 100
    tt1 = nn1 / fs1
    x1 = np.cos(tt1*2*np.pi * f1)
    ans1 = optimalZeropad(x1, fs1, f1)
    m2 = 210
    nn2 = np.arange(m2)
    fs2 = 10000.
    f2 = 250
    tt2 = nn2 / fs2
    x2 = np.cos(tt2*2*np.pi * f2)
    ans2 = optimalZeropad(x2, fs2, f2)
