...
def computeSNR(inputFile, window, M, N, H):
    ...
    y = stft.stft(x, w, N, H)
    SNR1 = 10*np.log10(signalE(x) / signalE(x-y))
    SNR2 = 10*np.log10(signalE(x[M:-M]) / signalE(x[M:-M]-y[M:-M]))
    return SNR1, SNR2

if __name__ == '__main__':
    inputFile = '../../sounds/piano.wav'
    SNR1 = computeSNR(inputFile, 'blackman', 513, 2048, 128)
    print SNR1, (67.57748352378475, 304.68394693221649)
    inputFile = '../../sounds/sax-phrase-short.wav'
    SNR2 = computeSNR(inputFile, 'hamming', 512, 1024, 64)
    print SNR2, (89.510506656299285, 306.18696700251388)
    inputFile = '../../sounds/rain.wav'
    SNR3 = computeSNR(inputFile, 'hann', 1024, 2048, 128)
    print SNR3, (74.631476225366825, 304.26918192997738)
